...
export class OrbitNavigation {
  ...
  reset() {
    this.active = false; this.orbiting = false; this.direction = 1;
  ...
  begin(bearing) {
    ...
    this.enemyPosition = { x: -offset.x, y: -offset.y, z: 6 - offset.z };
    this.updatePosition();
  }
  world(local) {
  ...
  attach(view) {
    ...
    this.anchor = { ...s.hit }; this.orbiting = false;
  ...
  getState() {
    return { active: this.active, orbiting: this.orbiting, direction: this.direction, radius: this.radius, angleDegrees: ((this.angle * 180 / Math.PI) % 360 + 360) % 360,
