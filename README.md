...
- Enter the 3D shoulder-camera crew fight, defeat the guard, collect supplies, and return to begin a new search.
...
- Pause, background tabs and focus loss clear held inputs and freeze combat. After victory the original fuel ×2 / ammunition ×1 / medical ×1 loot is collected once; returning resumes the next voyage. Defeat offers a clean retry or withdrawal. The voyage renderer is paused while boarding and the boarding renderer is created lazily.

Run `npm run test:boarding` with the local server for desktop/mobile real-input tests: crew joints, movement/look, mouse/touch chords, five-hit victory, death, loot/return, defeat/retry, pause and five viewport layouts. Screenshots are saved under ignored `qa-output/tps-*.png`. Supply the deployed URL as the script's first argument to verify the live build. Original pixel sprites remain in `assets/` as unused source artwork.
